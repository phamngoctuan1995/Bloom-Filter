...
/**
  ...
  */
class ScalableBloomFilter[T](maxElement: Long = 5000000000l, probFalsePositive: Double = 0.001, maxNumFilter: Int = 5) {
    ...
    var filters = List(new BloomFilter[T](filterSize, probPerFilter))
    ...
    def add(element: T): Unit = {
        if (countElement >= filterSize)
        {
            ...
            filters = new BloomFilter[T](filterSize, probPerFilter) :: filters
        }
